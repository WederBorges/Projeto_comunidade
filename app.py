...
def login():
    ...
    if form_login.validate_on_submit() and 'botao_submit_login' in request.form:
        flash(f"Login feito com sucesso no email: {form_login.email_login.data} !", 'sucess')
        print(request.form)
        return redirect(url_for('contato'))
        #Login com sucesso
    if form_criar_conta.validate_on_submit() and 'botao_submit_criar_conta' in request.form:
        flash(f"Cadastro realizado com sucesso para o email {form_criar_conta.email_cadastro.data}!", 'primary')
        return redirect(url_for('contato'))
